...
void setup() {
  ...
  player = minim.loadFile("cancion.mp3",512);
...
void draw() {
  ...
  if (!playing) { //Si es diferente de jugar entonces
    ...
    text("Juego de: P.P.G.", width/2, height/2 + 340);
    player.pause();
  ...
  if (playing) {//Cuando sea playing entonces
    ...
    fill(255,255,0);
    text(score, width/2, 90); //Mostrar marcador
    fill(0);
    text("By: Pablo Palma Garcia", width/2, 30); //Mostrar marcador
    player.play();
  }
}
